...
class M_stock extends CI_Model {
  ...
  public function liste_denree() {

    $q = "SELECT deleted,denree.id_denree, libelle,qte,unite,seuil,categorie from denree left join prix_denree on prix_denree.id_denree = denree.id_denree where categorie = 'denrée' and deleted is null order by libelle";
		$q = $this->db->query($q);
    ...
    $q = "SELECT deleted,denree.id_denree, libelle,qte,unite,seuil,categorie, prix from denree left join prix_denree on prix_denree.id_denree = denree.id_denree where categorie != 'denrée' and deleted is null order by libelle";
		$q = $this->db->query($q);
  ...
  public function get_information($id) {
		...
		$q = "SELECT c.creator, categorie, TO_CHAR(c.created,'dd TMMon YYYY à HH24:mi') as created, TO_CHAR(c.updated,'dd TMMon YYYY à HH24:mi') as updated, c.libelle, concat(z.nom,' ',z.prenom) as createur FROM denree c left join users z on c.creator = z.id_user WHERE c.id_denree = ?";
		$q = $this->db->query($q,array($id));
	...
	public function reset($dish) {

		$this->db->set('actif',0);
		return $this->db->where('id_denree',$dish)->update($this->table2);
	}
	...
	public function new_constitution($id,$id_constitute,$qte) {

		$this->db->set('id_denree',$id)
			->set('den_id_denree',$id_constitute)
			->set('qte',$qte);
